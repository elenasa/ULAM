...
namespace MFM {
  ...
  void NodeBlock::generateUlamClassInfo(File * fp, bool declOnly, u32& dmcount)
  {
    ...
    m_state.indent(fp);
    fp->write("UlamClassDataMemberInfo & ");
  ...
  void NodeBlock::generateUlamClassInfoCount(File * fp, bool declOnly, u32 dmcount)
  {
    //UTI cuti = m_state.getCompileThisIdx();
    UTI cuti = getNodeType();
    UlamType * cut = m_state.getUlamTypeByIndex(cuti);
