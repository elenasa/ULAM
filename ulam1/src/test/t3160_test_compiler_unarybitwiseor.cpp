...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3160_test_compiler_unarybitwiseor)
  {
    ...
    {
      return std::string("Ue_A { Unary(3) a(2);  Unary(3) b(1);  Unary(3) c(2);  Int(3) e(2);  Int(3) f(1);  Int(3) g(3);  Int(32) test() {  a e 2 cast = cast = b f 1 cast = cast = c a b | = g e f | = c cast return } }\nExit status: 2");
    }
