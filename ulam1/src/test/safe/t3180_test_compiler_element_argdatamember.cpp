...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3180_test_compiler_element_argdatamember)
  {
    ...
    {
      //Ue_Foo { Int(4) m_i(0);  Bool(1) m_b(false);  Int(32) test() {  Foo f;  f m_b . true cast = m_b f ( m_b )check . = f m_i . cast return } }\nExit status: 3
      return std::string("Ue_Foo { System s();  Bits(2) sp(0);  Bool(1) m_b(false);  Int(4) m_i(0);  Int(32) test() {  Foo f;  f m_b . true cast = m_b f ( m_b )check . = s ( m_i )print . s ( f m_i . )print . f m_i . cast return } }\nExit status: 3");
    }
