...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3148_test_compiler_elementmemberselect_returnarray)
  {
    ...
    {
      return std::string("Ue_Foo { typedef Bool BA[6];  Bool m_ba[6](false,true,false,true,false,true);  Int test() {  Foo f;  f m_ba 0 [] . true = f m_ba . f ( 1 )check . = m_ba f m_ba . = 0 return } }\nExit status: 0");
    }
