...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3171_test_compiler_typebitsizeexpression)
  {
    ...
    {
      return std::string("Ue_C { Bool(1) a(false);  Int(3) m_i[4](3,0,0,0);  Int(6) m_u(8);  Bool(3) b(false);  Int(32) test() {  Int(32) a;  a 3 cast = m_i 0 [] a cast = Int(5) exp;  exp 8 cast = m_u exp cast = m_i 1 [] exp cast = exp cast return } }\nExit status: 8");
    }
