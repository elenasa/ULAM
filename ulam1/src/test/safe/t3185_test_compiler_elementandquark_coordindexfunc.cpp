...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3185_test_compiler_elementandquark_coordindexfunc)
  {
    std::string GetAnswerKey()
    {
      ...
      return std::string("Ue_Foo { System s();  Bool(7) m_b(false);  Int(32) m_idx(40);  C2D m_coord( Int(6) m_width(9);  Int(6) m_height(4); );  Int(32) test() {  m_coord ( )init . m_idx m_coord ( 0 cast 0 cast )getIndex . = s ( m_idx )print . m_idx return } }\nExit status: 40");
    }
