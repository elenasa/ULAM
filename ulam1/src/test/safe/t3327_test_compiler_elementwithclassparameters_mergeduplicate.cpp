...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3327_test_compiler_elementwithclassparameters_mergeduplicate)
  {
    std::string GetAnswerKey()
    {
      ...
      return std::string("Exit status: 0\nUe_Foo { Bool(UNKNOWN) b(false);  constant Int(32) x = NONREADYCONST;  constant Unsigned(32) y = NONREADYCONST;  Bool(UNKNOWN) sp(false);  Bool(UNKNOWN) c(false);  Int(32) test() {  Foo(1,3u) f;  Foo(1,3u) p;  f sp . -2 -2 == = 0 return } }\nUe_Poo { Bool(1) valb[3](false,false,false);  <NOMAIN> }\n");
    }
