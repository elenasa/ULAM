...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3266_test_compiler_forloop_scope_issue)
  {
    std::string GetAnswerKey()
    {
	 ...
	 Int Arg: 0
      */
      return std::string("Exit status: 0\nUe_A { Bool(7) b(true);  System s();  Int(32) d(0);  Int(32) test() {  d 3 cast = { Int(32) index;  index 0 cast = index d < cond b index 0 cast != cast = _1: index 1 cast += while } { Unsigned(32) index;  index 0 cast = index 3u cast < cond { d 1 cast -= s ( d )print . } _2: index 1 cast += while } d return } }\nUq_System { <NOMAIN> }\n");
    }
