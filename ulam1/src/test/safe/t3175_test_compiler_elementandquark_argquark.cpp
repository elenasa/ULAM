...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3175_test_compiler_elementandquark_argquark)
  {
    ...
    {
      bool rtn1 = fms->add("Foo.ulam","ulam 1; use Bar; element Foo { Int(4) m_i; Bar m_bar; Bool check(Bar b) { return b.val_b[1] /* true */; } Int test() { Foo f;   f.m_bar.reset();   m_i = f.check(m_bar); return m_i; } }\n"); //worked without f.m_bar as arg to check func; m_i not true? is m_bar f's? YES..now it works.
