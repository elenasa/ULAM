...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3161_test_compiler_unarybitwisexor)
  {
    ...
    {
      return std::string("Ue_A { Unary(3) a(2);  Unary(3) b(1);  Unary(3) c(1);  Int(3) e(3);  Int(3) f(4);  Int(3) g(7);  Int(32) test() {  a e 3 cast = cast = b f 4 cast = cast = c a b ^ = g e f ^ = c cast return } }\nExit status: 1");
    }
