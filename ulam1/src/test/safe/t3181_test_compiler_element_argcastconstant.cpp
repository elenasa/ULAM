...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3181_test_compiler_element_argcastconstant)
  {
    ...
    {
      //Ue_Foo { Int(4) m_i(1);  Bool(1) m_b(true);  Int(32) test() {  m_i true cast = Bool(3) b;  b true cast = m_b ( true cast )check = m_b cast return } }\nExit status: 1
      return std::string("Ue_Foo { System s();  Bool(3) m_b(true);  Int(4) m_i(1);  Int(32) test() {  m_i true cast = Bool(3) b;  b true cast = m_b ( true cast )check cast = s ( m_b )print . m_b cast return } }\nExit status: 1");
    }
