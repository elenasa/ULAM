...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3360_test_compiler_elementandquarkswclassargs_memberconstantasstatement)
  {
    ...
    {
      return std::string("Exit status: 2\nUe_Foo { Int(32) test() {  Booly(0u,2u,true) b;  b 7u . b 2u . cast return } }\nUq_Booly { constant Unsigned(3) firstRange = NONREADYCONST;  constant Unsigned(3) lastRange = NONREADYCONST;  constant Bool(3) bomb = NONREADYCONST;  <NOMAIN> }\n");
    }
    ...
    std::string PresetTest(FileManagerString * fms)
    {
      ...
      bool rtn2 = fms->add("Booly.ulam","quark Booly(Unsigned(3) firstRange, Unsigned(3) lastRange, Bool(3) bomb) {\nBool behave(){\n if(bomb && lastRange != firstRange) return true;\nreturn false;\n}\n}\n");
