...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3158_test_compiler_booladd)
  {
    std::string GetAnswerKey()
    {
	 ...
	 Unary(3) Arg: 0x1
      */
      return std::string("Exit status: 1\nUe_A { Bool(3) b(false);  System s();  Bool(3) a(true);  Bool(3) c(true);  Unary(3) d(1);  Int(32) test() {  a true cast = b false cast = c a cast cast b cast cast +b 0u cast > cast = s ( c )print . d c cast = s ( d )print . c cast return } }\nUq_System { <NOMAIN> }\n");
    }
