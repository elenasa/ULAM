...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3217_test_compiler_elementandquark_elementparameterquark)
  {
    std::string GetAnswerKey()
    {
	 ...
	 Bool(3) Arg: 0x7 (true)
      */
      return std::string("Ue_Foo { Bool(3) b(true);  System s();  Bool(1) sp(false);  Bool(3) c(false);  Bool(1) last(false);  Int(32) test() {  barchance valb 1 [] . true cast = b barchance valb 1 [] . cast = s ( b )print . b cast return } }\nExit status: 1");
    }
