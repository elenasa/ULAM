...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3196_test_compiler_elementandquark_usercastquark)
  {
    std::string GetAnswerKey()
    {
      ...
      return std::string("Ue_Foo { System s();  Bool(3) sp(false);  Int(4) m_i(7);  Bar m_bar( Bool(1) val_b[3](false,true,false); );  Int(32) test() {  m_bar ( 1 cast )check = m_i m_bar ( )toInt . cast cast = s ( m_i )print . m_i cast return } }\nExit status: 7");
    }
