...
namespace MFM {
  ...
  BEGINTESTCASECOMPILER(t3195_test_compiler_element_bitstype_assignarray)
  {
    ...
    {
      // Both m_i and m_b show Bits? WRONG
      ...
      //I would have expected 2->true.
      return std::string("Ue_Foo { System s();  Bool(1) sp(false);  Bits(3) m_i[3](1,2,3);  Bool(3) m_b[3](false,true,true);  Int(32) test() {  m_i 0 [] 1 cast = m_i 1 [] 2 cast = m_i 2 [] m_i 0 [] cast m_i 1 [] cast +b cast = m_b m_i cast = s ( m_i 0 [] cast )print . s ( m_i 1 [] cast )print . s ( m_i 2 [] cast )print . s ( m_b 0 [] )print . s ( m_b 1 [] )print . s ( m_b 2 [] )print . m_b 2 [] cast return } }\nExit status: 1");
    }
