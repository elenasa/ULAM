...
namespace MFM {
  ...
  bool UlamTypeClassElement::needsImmediateType()
  {
	...
	if(cnsym && cnsym->isClassTemplate() && ((SymbolClassNameTemplate *) cnsym)->isClassTemplate(cuti))
  ...
  void UlamTypeClassElement::genUlamTypeAutoWriteDefinitionForC(File * fp)
  {
    ...
    fp->write("& targ) { ");
    if(isScalar())
      fp->write("if(targ.GetType() != Us::ELEMENT_TYPE) FAIL(ILLEGAL_ARGUMENT); ");
    fp->write("UlamRef<EC>::");
	...
	fp->write("& v) { ");
	fp->write("if(v.GetType() != Us::ELEMENT_TYPE) FAIL(ILLEGAL_ARGUMENT); ");
	fp->write("UlamRef<EC>::");
	fp->write(writeMethodForCodeGen().c_str());
  ...
  void UlamTypeClassElement::genUlamTypeMangledDefinitionForC(File * fp)
  {
    ...
    fp->write("AtomBitStorage<EC>");
    fp->write("(d) { ");
    fp->write("if(d.GetType() != Us::ELEMENT_TYPE) FAIL(ILLEGAL_ARGUMENT);");
    fp->write(" }"); GCNL;
    ...
    fp->write("if(arg.GetType() != Us::ELEMENT_TYPE) FAIL(ILLEGAL_ARGUMENT);");
    fp->write(" }"); GCNL;
  ...
  void UlamTypeClassElement::genUlamTypeMangledUnpackedArrayDefinitionForC(File * fp)
  {
   ...
   fp->write(" : public ");
    ...
    // assignment constructor (e.g. t3706,t3710)
    m_state.indent(fp);
    fp->write(mangledName.c_str());
    fp->write("(const ");
    fp->write(mangledName.c_str());
