...
namespace MFM {
  ...
  const char * NodeBinaryOpCompareLessThan::getInverseOpName()
  {
    return ">=";
  }
  ...
  UlamValue NodeBinaryOpCompareLessThan::makeImmediateLongBinaryOp(UTI type, u64 ldata, u64 rdata, u32 len)
  {
    UlamValue rtnUV;
    UTI nuti = getNodeType(); //Bool
    u32 nodelen = m_state.getBitSize(nuti);
    ...
    switch(typEnum)
      {
      case Int:
	rtnUV = UlamValue::makeImmediateLong(nuti, _BinOpCompareLessThanInt64(ldata, rdata, len), nodelen);
	break;
      case Unsigned:
	rtnUV = UlamValue::makeImmediateLong(nuti, _BinOpCompareLessThanUnsigned64(ldata, rdata, len), nodelen);
	break;
      case Bool:
	rtnUV = UlamValue::makeImmediateLong(nuti, _BinOpCompareLessThanBool64(ldata, rdata, len), nodelen);
	break;
      case Unary:
	rtnUV = UlamValue::makeImmediateLong(nuti, _BinOpCompareLessThanUnary64(ldata, rdata, len), nodelen);
	break;
      case String:
	m_state.abortNotImplementedYet(); //not implemented yet!
	break;
