...
namespace MFM {
  ...
  {
    //if condition is a constant, surgery .. TODO
    return getNodeType();
  }
  ...
  void NodeQuestionColon::genCode(File * fp, UVPass& uvpass)
  {
    ...
    if(nuti != Void)
      {
	Node::genCodeConvertATmpVarIntoBitVector(fp, ruvpass);
  ...
  void NodeQuestionColon::genCodeToStoreInto(File * fp, UVPass& uvpass)
  {
	...
	//if(!cossym->isTmpVarSymbol()) makes ulamexports EVentWindow very unhappy!
	{
	  ...
	  Node::genCodeReferenceInitialization(fp, luvpass, tmpvarsym);
	  delete tmpvarsym;
	  uvpass = luvpass;
	}
#endif
