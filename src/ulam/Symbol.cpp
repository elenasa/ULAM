...
namespace MFM {
  ...
  Symbol::Symbol(const Token& id, UTI utype, CompilerState & state) : m_state(state), m_gotStructuredCommentToken(false), m_idtok(id), m_uti(utype), m_dataMemberClass(Nouti), m_localfilescopeType(Nouti), m_autoLocalType(ALT_NOT), m_isSelf(false), m_isSuper(false), m_stBlockNo(state.getCurrentBlockNo()){}

  Symbol::Symbol(const Symbol & sref) : m_state(sref.m_state), m_structuredCommentToken(sref.m_structuredCommentToken), m_gotStructuredCommentToken(sref.m_gotStructuredCommentToken), m_idtok(sref.m_idtok), m_uti(m_state.mapIncompleteUTIForCurrentClassInstance(sref.m_uti)), m_dataMemberClass(m_state.mapIncompleteUTIForCurrentClassInstance(sref.m_dataMemberClass)), m_localfilescopeType(sref.m_localfilescopeType), m_autoLocalType(sref.m_autoLocalType), m_isSelf(sref.m_isSelf), m_isSuper(sref.m_isSuper), m_stBlockNo(sref.m_stBlockNo) {}

  Symbol::Symbol(const Symbol& sref, bool keepType) : m_state(sref.m_state), m_structuredCommentToken(sref.m_structuredCommentToken), m_gotStructuredCommentToken(sref.m_gotStructuredCommentToken), m_idtok(sref.m_idtok), m_uti(sref.m_uti), m_dataMemberClass(sref.m_dataMemberClass), m_localfilescopeType(sref.m_localfilescopeType), m_autoLocalType(sref.m_autoLocalType), m_isSelf(sref.m_isSelf), m_isSuper(sref.m_isSuper), m_stBlockNo(sref.m_stBlockNo) {}
  ...
  void Symbol::setLocalFilescopeDef(UTI locuti)
  {
    m_localfilescopeType = locuti;
  }

  UTI Symbol::getLocalFilescopeType()
  {
    assert(isLocalFilescopeDef());
    return m_localfilescopeType;
  }

  bool Symbol::isLocalFilescopeDef()
  {
    return (m_localfilescopeType != Nouti);
  }
